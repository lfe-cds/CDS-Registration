...
The requirements for hosting this metadata endpoint follows [OAuth 2.0 Authorization Server Metadata](#ref-oauth2-server-metadata) with several additions for Connected Data Specifications (CDS) functionality described in the following sections.

### 3.1. Metadata URL <a id="auth-server-metadata-url" href="#auth-server-metadata-url" class="permalink">🔗</a>

Following OAuth's [Obtaining Authorization Server Metadata](#ref-oauth2-server-metadata-req) process, a Server's Authorization Server Metadata Object is retrieved by making an HTTPS request to a specified URL.

To ensure discoverability to the specified URL, this specification extends [CDS-WG1-01 Server Capabilities](/specs/cds-wg1-01/#server-capabilities) to require the following values:
...
As well as extending the [CDS-WG1-01 Metadata Object Format](/specs/cds-wg1-01/#metadata-object-format) to require the following values:

* `oauth_metadata` - _[URL](#url)_ - (REQUIRED) Where Clients can obtain the Server's Authorization Server Metadata Object

In addition to requiring that the URL be included in the [CDS-WG1-01 Metadata Object Format](/specs/cds-wg1-01/#metadata-object-format), Servers MAY also configure the URL such that it conforms to the default [Well-Known URI](#ref-oauth2-server-metadata-wellknown) format, which can increase interoperability beyond the above specified CDS-WG1-01 extension.

### 3.2. Metadata Object Format <a id="auth-server-metadata-format" href="#auth-server-metadata-format" class="permalink">🔗</a>

A Server's Authorization Server Metadata Object follows OAuth's [Authorization Server Metadata](#ref-oauth2-server-metadata-obj) object format with the following modifications from OPTIONAL or RECOMMENDED to REQUIRED:

* `registration_endpoint` - _[URL](#url)_ - (REQUIRED) OAuth's [Dynamic Client Registration](#ref-oauth2-dynamic-client-reg) functionality is required to enable the [Client Registration Process](#client-registration-process).
...
* `revocation_endpoint` - _[URL](#url)_ - (REQUIRED) OAuth's [Token Revocation](#ref-oauth2-token-revocation) functionality is required.
...
In addition to the standard set of OAuth [Authorization Server Metadata](#ref-oauth2-server-metadata-obj) values, this specification also requires the following OAuth extension capabilities to be included in the metadata object values:

* `authorization_details_types_supported` - _Array[[string](#string)]_ - (REQUIRED) OAuth's [Rich Authorization Requests](#ref-oauth2-rar) functionality is required.
...
In addition to the above additionally required set of OAuth [Authorization Server Metadata](#ref-oauth2-server-metadata-obj) values, this specification clarifies use of the following OAuth standard values:
...
* `response_types_supported` - _Array[[string](#string)]_ - (REQUIRED) The OAuth `response_type` values that can be used with this scope.
  This value follows the same behavior as OAuth's [Metadata object `response_types_supported`](#ref-oauth2-server-metadata-obj), except that this value represents the response types supported for this individual scope and not all scopes for the Client.
...
* `grant_types_supported` - _Array[[string](#string)]_ - (REQUIRED) The OAuth grant type values that can be used with this scope.
  This value follows the same behavior as OAuth's [Metadata object `grant_types_supported`](#ref-oauth2-server-metadata-obj), except that this value represents the grant types supported for this individual scope and not all scopes for the Client.
...
* `token_endpoint_auth_methods_supported` - _Array[[string](#string)]_ - (REQUIRED) The OAuth client authentication methods that can be used for granting tokens with this scope.
  This value follows the same behavior as OAuth's [Metadata object `token_endpoint_auth_methods_supported`](#ref-oauth2-server-metadata-obj), except that this value represents the client authentication methods supported for this individual scope and not all scopes for the Client.
...
* `coverages_supported` - _Array[[string](#string)]_ - (REQUIRED) A list of [CDS-WG1-01 Coverage Entry Object](/specs/cds-wg1-01/#coverage-entry-format) `id` values for which the scope is available by the Server.
...
* `authorization_details_fields_supported` - _Array[[AuthorizationDetailsField](#auth-details-fields-format)]_ - (REQUIRED) A list of fields that MAY be included in [Rich Authorization Requests](#ref-oauth2-rar) the authorization details object for this scope `type`.
...
* `id` - _[string](#string)_ - (REQUIRED) The unique identifier of the authorization details field.
  This value is used as the key for the field when added to `authorization_details` data fields as part of OAuth's [Rich Authorization Requests](#ref-oauth2-rar).
...
Authorization Details Field formats define the data type of submitted values for these fields in `authorization_details` for OAuth's [Rich Authorization Requests](#ref-oauth2-rar).
...
OAuth's [Dynamic Client Registration](#ref-oauth2-dynamic-client-reg) is used as the basis for initially registering Clients with Servers.
...
This specification requires Clients and Servers follow the process described in OAuth's [Client Registration Request](#ref-oauth2-client-reg-request), with the following modifications.
...
This specification requires Servers follow the process described in OAuth's [Client Registration Response](#ref-oauth2-client-reg-resp), with the following modifications to OAuth's [Client Information Response](#ref-oauth2-client-info-resp) object.
...
These APIs are authenticated using a Bearer `access_token` obtained by the Client using OAuth 2.0's `client_credentials` grant ([Section 4.4](#ref-oauth2-client-credentials))) process, where the `client_id` used is for a Client that includes the `client_admin` scope.
...
Client objects are required to follow the same object format as the OAuth Dynamic Client Registration Management Protocol's [Client Metadata](#ref-oauth2-client-metadata), with the following modifications.

The following fields defined in OAuth's [Client Metadata](#ref-oauth2-client-metadata) are REQUIRED to be included with all Client objects:
...
The following fields defined in OAuth's [Rich Authorization Requests Metadata](#ref-oauth2-rar-metadata) are REQUIRED to be included with all Client objects:
...
Other fields defined in the OAuth's [Client Metadata](#ref-oauth2-client-metadata) and its extensions, such as OAuth's Dynamic Client Registration Management Protocol [Client Information Response](#ref-oauth2-client-mgmt-metadata), remain unmodified in their requirement and behavior.

In addition to the fields defined by OAuth's [Client Metadata](#ref-oauth2-client-metadata) and its extensions, the following fields are additionally defined:
...
* `cds_server_metadata` - _[URL](#url)_ - (REQUIRED) Where the Client can find their registration-specific version of the [CDS-WG1-01 Server Metadata](/specs/cds-wg1-01/).
...
* `cds_default_authorization_details` - _Array[[OAuth AuthorizationDetail](#ref-oauth2-rar-auth-details)]_ - (OPTIONAL) The default authorization details list used when no `authorization_details` parameter is provided as part of an authorization request.
  This MUST be included if `response_types` is not an empty list (i.e. authorization requests are enabled).
...
If a Server has optionally implemented OAuth's [Dynamic Client Registration Management Protocol](#ref-oauth2-client-mgmt), the value of `cds_client_uri` MUST be the same as `registration_client_uri`, and access tokens issued from either a `client_credentials` grant with the scope `client_admin` or the access token provided as the `registration_access_token` MUST be valid access tokens to interact with the client retrieval endpoint (`cds_client_uri`).

### 5.5. Modifying Clients <a id="clients-modify" href="#clients-modify" class="permalink">🔗</a>

This specification requires that the procedure to modify Clients MUST follow OAuth's [Client Update Request](#ref-oauth2-client-mgmt-updates) section in [Dynamic Client Registration Management Protocol](#ref-oauth2-client-mgmt).
...
The Messages API endpoints are authenticated using a Bearer `access_token` obtained by the Client using OAuth 2.0's `client_credentials` grant ([Section 4.4](#ref-oauth2-client-credentials))) process, where the `client_id` used is for a Client that includes the `client_admin` scope.
...
* `authorization_details` - _Array[[OAuth AuthorizationDetail](#ref-oauth2-rar-auth-details)]_ - (OPTIONAL) The OAuth authorization details, if any, to further specify the grant's requested scope.
...
The Credentials API endpoints are authenticated using a Bearer `access_token` obtained by the Client using OAuth 2.0's `client_credentials` grant ([Section 4.4](#ref-oauth2-client-credentials))) process, where the `client_id` used is for a Client that includes the `client_admin` scope.
...
* `client_secret` - _[string](#string)_ - (OPTIONAL) A sufficiently random value generated by the Server that can be used by the Client to authenticate themselves when obtaining an `access_token` from the Server's OAuth 2.0 [Token Endpoint](#ref-oauth2-token-endpoint).
  This field MUST be included if the `type` is `client_secret`.
* `client_secret_expires_at` - _[integer](#integer)_ - (OPTIONAL) The timestamp at which the `client_secret` will expire, as defined in OAuth's [Client Information Response](#ref-oauth2-client-info-resp).
...
* `authorization_details` - _Array[[OAuth AuthorizationDetail](#ref-oauth2-rar-auth-details)]_ - (REQUIRED) Any authorization details scopes that are granted in addition to this object's `scope` value.
...
* `enabled_authorization_details` - _Array[[OAuth AuthorizationDetail](#ref-oauth2-rar-auth-details)]_ - For Grants where access has been partially granted, but some access is still disabled, this value is the `authorization_details` that has been enabled by the server.
...
These APIs are authenticated using a Bearer `access_token` obtained by the Client using OAuth 2.0's `client_credentials` grant ([Section 4.4](#ref-oauth2-client-credentials))) process, where the scope of the access token is `grant_admin` with `authorizatin_details` entries listing `grant_id` values that are for Grants that have the `server_provided_files` scope.
...
If [Authorization Server Metadata](#auth-server-metadata) is referenced in a public [CDS-WG1-01 Server Metadata endpoint](/specs/cds-wg1-01/#metadata-endpoint), then the [Authorization Server Metadata](#auth-server-metadata) and [Client Registration Process](#client-registration-process) must also be public.
...
For authenticated endpoints ([Clients API](#clients-api), [Messages API](#messages-api), [Credentials API](#credentials-api), [Grants API](#grants-api)), Servers MUST authenticate requests using OAuth's [Authorization Request Header Field](#ref-oauth2-auth-header) with access tokens obtained using the OAuth 2.0's Issuing an Access Token ([Section 5](#ref-oauth2-access-tokens)) process.
...
<a id="ref-base64" href="#ref-base64" class="permalink">🔗</a>
`Base64` - "The Base16, Base32, and Base64 Data Encodings", RFC 4648, Internet Engineering Task Force (IETF),
...
<a id="ref-oauth2" href="#ref-oauth2" class="permalink">🔗</a>
`OAuth 2.0` - "The OAuth 2.0 Authorization Framework", RFC 6749, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc6749](https://www.rfc-editor.org/rfc/rfc6749)

<a id="ref-oauth2-token-endpoint" href="#ref-oauth2-token-endpoint" class="permalink">🔗</a>
`OAuth 2.0 Token Endpoint` - Section 3.2: Token Endpoint, "The OAuth 2.0 Authorization Framework", RFC 6749, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc6749#section-3.2](https://www.rfc-editor.org/rfc/rfc6749#section-3.2)

<a id="ref-oauth2-client-credentials" href="#ref-oauth2-client-credentials" class="permalink">🔗</a>
`OAuth 2.0 Client Credentials Grant` - Section 4.4. Client Credentials Grant, "The OAuth 2.0 Authorization Framework", RFC 6749, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc6749#section-4.4](https://www.rfc-editor.org/rfc/rfc6749#section-4.4)

<a id="ref-oauth2-access-tokens" href="#ref-oauth2-access-tokens" class="permalink">🔗</a>
`OAuth 2.0 Issuing Access Tokens` - Section 5. Issuing an Access Token, "The OAuth 2.0 Authorization Framework", RFC 6749, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc6749#section-5](https://www.rfc-editor.org/rfc/rfc6749#section-5)

<a id="ref-oauth2-auth-header" href="#ref-oauth2-auth-header" class="permalink">🔗</a>
`OAuth Authorization Request Header Field` - Section 2.1. Authorization Request Header Field, "The OAuth 2.0 Authorization Framework: Bearer Token Usage", RFC 6750, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc6750#section-2.1](https://www.rfc-editor.org/rfc/rfc6750#section-2.1)

<a id="ref-oauth2-dynamic-client-reg" href="#ref-oauth2-dynamic-client-reg" class="permalink">🔗</a>
`Dynamic Client Registration` - "OAuth 2.0 Dynamic Client Registration Protocol", RFC 7591, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7591](https://www.rfc-editor.org/rfc/rfc7591)

<a id="ref-oauth2-client-metadata" href="#ref-oauth2-client-metadata" class="permalink">🔗</a>
`Client Metadata` - Section 2. Client Metadata, "OAuth 2.0 Dynamic Client Registration Protocol", RFC 7591, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7591#section-2](https://www.rfc-editor.org/rfc/rfc7591#section-2)

<a id="ref-oauth2-client-reg-request" href="#ref-oauth2-client-reg-request" class="permalink">🔗</a>
`Client Registration Request` - Section 3.1. Client Registration Request, "OAuth 2.0 Dynamic Client Registration Protocol", RFC 7591, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7591#section-3.1](https://www.rfc-editor.org/rfc/rfc7591#section-3.1)

<a id="ref-oauth2-client-reg-resp" href="#ref-oauth2-client-reg-resp" class="permalink">🔗</a>
`Client Registration Response` - Section 3.2. Responses, "OAuth 2.0 Dynamic Client Registration Protocol", RFC 7591, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7591#section-3.2](https://www.rfc-editor.org/rfc/rfc7591#section-3.2)

<a id="ref-oauth2-client-info-resp" href="#ref-oauth2-client-info-resp" class="permalink">🔗</a>
`Client Information Response` - Section 3.2.1. Client Information Response, "OAuth 2.0 Dynamic Client Registration Protocol", RFC 7591, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7591#section-3.2.1](https://www.rfc-editor.org/rfc/rfc7591#section-3.2.1)

<a id="ref-oauth2-server-metadata" href="#ref-oauth2-server-metadata" class="permalink">🔗</a>
`OAuth Authorization Server Metadata` - "OAuth 2.0 Authorization Server Metadata", RFC 8414, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc8414](https://www.rfc-editor.org/rfc/rfc8414)

<a id="ref-oauth2-server-metadata-obj" href="#ref-oauth2-server-metadata-obj" class="permalink">🔗</a>
`OAuth Authorization Server Metadata Object` - Section 2. Authorization Server Metadata, "OAuth 2.0 Authorization Server Metadata", RFC 8414, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc8414#section-2](https://www.rfc-editor.org/rfc/rfc8414#section-2)

<a id="ref-oauth2-server-metadata-req" href="#ref-oauth2-server-metadata-req" class="permalink">🔗</a>
`Obtaining Authorization Server Metadata` - Section 3. Obtaining Authorization Server Metadata, "OAuth 2.0 Authorization Server Metadata", RFC 8414, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc8414#section-3](https://www.rfc-editor.org/rfc/rfc8414#section-3)

<a id="ref-oauth2-server-metadata-wellknown" href="#ref-oauth2-server-metadata-wellknown" class="permalink">🔗</a>
`Server Metadata Well-Known URI` - Section 7.3. Well-Known URI Registry, "OAuth 2.0 Authorization Server Metadata", RFC 8414, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc8414#section-7.3](https://www.rfc-editor.org/rfc/rfc8414#section-7.3)

<a id="ref-oauth2-token-revocation" href="#ref-oauth2-token-revocation" class="permalink">🔗</a>
`Token Revocation` - "OAuth 2.0 Token Revocation", RFC 7009, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7009](https://www.rfc-editor.org/rfc/rfc7009)

<a id="ref-oauth2-rar" href="#ref-oauth2-rar" class="permalink">🔗</a>
`Rich Authorization Requests` - "OAuth 2.0 Rich Authorization Requests", RFC 9396, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc9396](https://www.rfc-editor.org/rfc/rfc9396)

<a id="ref-oauth2-rar-auth-details" href="#ref-oauth2-rar-auth-details" class="permalink">🔗</a>
`Authorization Details` - Section 7.1. Enriched Authorization Details in Token Response, "OAuth 2.0 Rich Authorization Requests", RFC 9396, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc9396#section-7.1](https://www.rfc-editor.org/rfc/rfc9396#section-7.1)

<a id="ref-oauth2-rar-metadata" href="#ref-oauth2-rar-metadata" class="permalink">🔗</a>
`Rich Authorization Requests Metadata` - Section 10. Metadata, "OAuth 2.0 Rich Authorization Requests", RFC 9396, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc9396#section-10](https://www.rfc-editor.org/rfc/rfc9396#section-10)

<a id="ref-oauth2-client-mgmt" href="#ref-oauth2-client-mgmt" class="permalink">🔗</a>
`Client Registration Management` - "OAuth 2.0 Dynamic Client Registration Management Protocol", RFC 7592, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7592](https://www.rfc-editor.org/rfc/rfc7592)

<a id="ref-oauth2-client-mgmt-updates" href="#ref-oauth2-client-mgmt-updates" class="permalink">🔗</a>
`Client Update Request` - Section 2.2. Client Update Request, "OAuth 2.0 Dynamic Client Registration Management Protocol", RFC 7592, Internet Engineering Task Force (IETF),
[https://www.rfc-editor.org/rfc/rfc7592#section-2.2](https://www.rfc-editor.org/rfc/rfc7592#section-2.2)

<a id="ref-oauth2-client-mgmt-metadata" href="#ref-oauth2-client-mgmt-metadata" class="permalink">🔗</a>
`Client Information Response` - Section 3. Client Information Response, "OAuth 2.0 Dynamic Client Registration Management Protocol", RFC 7592, Internet Engineering Task Force (IETF),
