...
As we write, discuss, and publish our [specifications]({{ '/specs' | relative_url }}) for how to provide streamlined, secure registration and connectivity to address our target [Use Cases]({{ '/use-cases' | relative_url }}), we will follow several design principles that help guide our choices as a working group.
...
Our specifications will define how a server must operate so that a client user can integrate with it to access data and functionality, so that clients may perform services for utilities, centralized entities, and customers that advance the energy ecosystem.
